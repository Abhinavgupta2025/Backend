...
import { WarningPriority, debugLogger, coreEvents, AuthType, } from '@google/gemini-cli-core';
...
vi.mock('./utils/terminalNotifications.js', () => ({
    ...
    buildRunEventNotificationContent: terminalNotificationMocks.buildRunEventNotificationContent,
}));
...
describe('gemini.tsx main function kitty protocol', () => {
    ...
    it('should read from stdin in non-interactive mode', async () => {
        ...
        expect(terminalNotificationMocks.buildRunEventNotificationContent).not.toHaveBeenCalled();
...
describe('gemini.tsx main function exit codes', () => {
    ...
    it('should validate and refresh auth in non-interactive mode when no auth type is selected but env var is present', async () => {
        ...
        vi.mocked(validateNonInteractiveAuth).mockResolvedValue(AuthType.USE_GEMINI);
        vi.mocked(loadSettings).mockReturnValue(createMockSettings({
...
describe('project hooks loading based on trust', () => {
    ...
    beforeEach(async () => {
        ...
        loadCliConfig.mockResolvedValue(createMockConfig({
    ...
    it('should load project hooks when workspace is trusted', async () => {
        ...
        loadSettings.mockReturnValue(createMockSettings({
            workspace: {
                isTrusted: true,
                settings: { hooks },
            },
        ...
        await main();
        expect(loadCliConfig).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.anything(), expect.objectContaining({
            projectHooks: hooks,
        }));
    });
