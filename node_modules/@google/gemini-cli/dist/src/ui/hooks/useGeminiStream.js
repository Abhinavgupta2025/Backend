...
import { GeminiEventType as ServerGeminiEventType, getErrorMessage, isNodeError, MessageSenderType, logUserPrompt, GitService, UnauthorizedError, UserPromptEvent, DEFAULT_GEMINI_FLASH_MODEL, logConversationFinishedEvent, ConversationFinishedEvent, ApprovalMode, parseAndFormatApiError, ToolConfirmationOutcome, MessageBusType, promptIdContext, tokenLimit, debugLogger, runInDevTraceSpan, EDIT_TOOL_NAMES, ASK_USER_TOOL_NAME, processRestorableToolCalls, recordToolCallInteractions, ToolErrorType, ValidationRequiredError, coreEvents, CoreEvent, CoreToolCallStatus, buildUserSteeringHintPrompt, generateSteeringAckMessage, getPlanModeExitMessage, } from '@google/gemini-cli-core';
...
export const useGeminiStream = (geminiClient, history, addItem, config, settings, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onCancelSubmit, setShellInputFocused, terminalWidth, terminalHeight, isShellFocused, consumeUserHint) => {
        ...
        if (toolsToPush.length > 0) {
            ...
            for (const tc of toolsToPush) {
                ...
                const historyItem = mapTrackedToolCallsToDisplay(tc, {
                    ...
                    ...getToolGroupBorderAppearance({ type: 'tool_group', tools: toolCalls }, activeShellPtyId, !!isShellFocused, [], backgroundShells),
                });
                addItem(historyItem);
                isFirst = false;
            }
    ...
    const pendingToolGroupItems = useMemo(() => {
        ...
        const items = [];
        const appearance = getToolGroupBorderAppearance({ type: 'tool_group', tools: toolCalls }, activeShellPtyId, !!isShellFocused, [], backgroundShells);
    ...
    const handleCompletedTools = useCallback(async (completedToolCallsFromScheduler) => {
        ...
        if (consumeUserHint) {
            ...
            if (userHint && userHint.trim().length > 0) {
                ...
                void generateSteeringAckMessage(config.getBaseLlmClient(), hintText).then((ackText) => {
                    addItem({
                        type: 'info',
                        icon: '· ',
