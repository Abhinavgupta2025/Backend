...
import { CoreToolCallStatus, ApprovalMode, AuthType, GeminiEventType as ServerGeminiEventType, ToolErrorType, ToolConfirmationOutcome, MessageBusType, tokenLimit, debugLogger, coreEvents, CoreEvent, MCPDiscoveryState, getPlanModeExitMessage, } from '@google/gemini-cli-core';
...
describe('useGeminiStream', () => {
    ...
    it('should inject steering hint prompt for continuation', async () => {
        ...
        renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24, undefined, () => 'focus on tests only'));
        await act(async () => {
            if (capturedOnComplete) {
                await new Promise((resolve) => setTimeout(resolve, 0));
                await capturedOnComplete(completedToolCalls);
        ...
        expect(mockAddItem.mock.calls.some(([item]) => item?.type === 'info' &&
            typeof item.text === 'string' &&
            item.text.includes('Got it. Focusing on tests only.'))).toBe(true);
    });
