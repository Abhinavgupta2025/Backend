...
export const DialogManager = ({ addItem, terminalWidth, }) => {
    ...
    if (uiState.isFolderTrustDialogOpen) {
        return (_jsx(FolderTrustDialog, { onSelect: uiActions.handleFolderTrustSelect, isRestarting: uiState.isRestarting, discoveryResults: uiState.folderDiscoveryResults }));
    }
    if (uiState.isPolicyUpdateDialogOpen) {
        return (_jsx(PolicyUpdateDialog, { config: config, request: uiState.policyUpdateConfirmationRequest, onClose: () => uiActions.setIsPolicyUpdateDialogOpen(false) }));
    }
    ...
    if (uiState.accountSuspensionInfo) {
        return (_jsx(Box, { flexDirection: "column", children: _jsx(BannedAccountDialog, { accountSuspensionInfo: uiState.accountSuspensionInfo, onExit: () => {
                    process.exit(1);
                }, onChangeAuth: () => {
