...
export class ToolExecutor {
        ...
        return runInDevTraceSpan({
            ...
            try {
                ...
                if (invocation instanceof ShellToolInvocation) {
                    ...
                    promise = executeToolWithHooks(invocation, toolName, signal, tool, liveOutputCallback, shellExecutionConfig, setPidCallback, this.config, request.originalRequestName);
                }
                else {
                    promise = executeToolWithHooks(invocation, toolName, signal, tool, liveOutputCallback, shellExecutionConfig, undefined, this.config, request.originalRequestName);
                }
                ...
                if (signal.aborted) {
                    ...
                    const displayText = typeof toolResult.returnDisplay === 'string'
                        ...
                        : undefined;
                    return this.createErrorResult(call, new Error(toolResult.error.message), toolResult.error.type, displayText, toolResult.tailToolCallRequest);
                }
            }
    ...
    async createSuccessResult(call, toolResult) {
        ...
        if (typeof content === 'string' && toolName === SHELL_TOOL_NAME) {
            ...
            if (threshold > 0 && content.length > threshold) {
                ...
                const { outputFile: savedPath } = await saveTruncatedToolOutput(content, toolName, callId, this.config.storage.getProjectTempDir(), this.config.getSessionId());
    ...
    createErrorResult(call, error, errorType, returnDisplay, tailToolCallRequest) {
