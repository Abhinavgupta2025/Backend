    ...
    describe('HookCallEvent', () => {
        describe('constructor', () => {
            it('should create an event with all fields', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'test-hook', { tool_name: 'ReadFile' }, 100, true, { decision: 'allow' }, 0, 'output', 'error', undefined);
            ...
            it('should create an event with minimal fields', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'test-hook', { tool_name: 'ReadFile' }, 100, true);
        ...
        describe('toOpenTelemetryAttributes with logPrompts=true', () => {
            ...
            it('should include all fields when logPrompts is enabled', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, '/path/to/.gemini/hooks/check-secrets.sh --api-key=abc123', { tool_name: 'ReadFile', args: { file: 'secret.txt' } }, 100, true, { decision: 'allow' }, 0, 'hook executed successfully', 'no errors');
            ...
            it('should include hook_input and hook_output as JSON strings', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'test-hook', { tool_name: 'ReadFile', args: { file: 'test.txt' } }, 100, true, { decision: 'allow', reason: 'approved' });
                const attributes = event.toOpenTelemetryAttributes(config);
        ...
        describe('toOpenTelemetryAttributes with logPrompts=false', () => {
            ...
            it('should sanitize hook_name when logPrompts is disabled', () => {
                ...
                for (const testCase of testCases) {
                    const event = new HookCallEvent('BeforeTool', HookType.Command, testCase.input, { tool_name: 'ReadFile' }, 100, true);
                    const attributes = event.toOpenTelemetryAttributes(config);
                    expect(attributes['hook_name']).toBe(testCase.expected);
                }
            });
            it('should still include error field even when logPrompts is disabled', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'test-hook', { tool_name: 'ReadFile' }, 100, false, undefined, undefined, undefined, undefined, 'Hook execution failed');
                const attributes = event.toOpenTelemetryAttributes(config);
        ...
        describe('sanitizeHookName edge cases', () => {
            ...
            it('should handle commands with multiple spaces', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'python   script.py   --arg1   --arg2', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('python');
            });
            it('should handle mixed path separators', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, '/path/to\\mixed\\separators.sh', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('separators.sh');
            });
            it('should handle trailing slashes', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, '/path/to/directory/', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
        ...
        describe('toLogBody', () => {
            it('should format success message correctly', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'test-hook', {}, 150, true);
                expect(event.toLogBody()).toBe('Hook call BeforeTool.test-hook succeeded in 150ms');
            });
            it('should format failure message correctly', () => {
                const event = new HookCallEvent('AfterTool', HookType.Command, 'validation-hook', {}, 75, false);
        ...
        describe('integration scenarios', () => {
            it('should handle enterprise scenario with full logging', () => {
                ...
                const event = new HookCallEvent('BeforeModel', HookType.Command, '$GEMINI_PROJECT_DIR/.gemini/hooks/add-context.sh', {
                    llm_request: {
        ...
        describe('real-world sensitive command examples', () => {
            ...
            it('should sanitize commands with API keys', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'curl https://api.example.com -H "Authorization: Bearer sk-abc123xyz"', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('curl');
            });
            it('should sanitize commands with database credentials', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'psql postgresql://user:password@localhost/db', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('psql');
            });
            it('should sanitize commands with environment variables containing secrets', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'AWS_SECRET_KEY=abc123 aws s3 ls', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('AWS_SECRET_KEY=abc123');
            });
            it('should sanitize Python scripts with file paths', () => {
                const event = new HookCallEvent('BeforeTool', HookType.Command, 'python /home/john.doe/projects/secret-scanner/scan.py --config=/etc/secrets.yml', {}, 100, true);
                const attributes = event.toOpenTelemetryAttributes(config);
                expect(attributes['hook_name']).toBe('python');
            });
        });
