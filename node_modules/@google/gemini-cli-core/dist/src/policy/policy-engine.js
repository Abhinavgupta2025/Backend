...
function getToolMetadata(toolName, serverName) {
    ...
    return {
        actualServer: serverName ?? (isQualified ? toolName.substring(0, sepIndex) : undefined),
...
export class PolicyEngine {
     ...
     */
    async checkShellCommand(toolName, command, ruleDecision, serverName, dir_path, allowRedirection, rule, toolAnnotations) {
        if (!command) {
            return {
        ...
        if (subCommands.length > 0) {
            ...
            for (const rawSubCmd of subCommands) {
                ...
                const subResult = await this.check({ name: toolName, args: { command: subCmd, dir_path } }, serverName, toolAnnotations);
    ...
    async check(toolCall, serverName, toolAnnotations) {
        ...
        for (const rule of this.rules) {
            const match = toolCallsToTry.some((tc) => ruleMatches(rule, tc, stringifiedArgs, serverName, this.approvalMode, toolAnnotations));
            if (match) {
                ...
                if (isShellCommand && toolName) {
                    const shellResult = await this.checkShellCommand(toolName, command, rule.decision, serverName, shellDirPath, rule.allowRedirection, rule, toolAnnotations);
        ...
        if (decision === undefined) {
            ...
            if (toolName && SHELL_TOOL_NAMES.includes(toolName)) {
                const shellResult = await this.checkShellCommand(toolName, command, this.defaultDecision, serverName, shellDirPath, undefined, undefined, toolAnnotations);
                decision = shellResult.decision;
                matchedRule = shellResult.rule;
            }
            else {
        ...
        if (decision !== PolicyDecision.DENY && this.checkerRunner) {
            for (const checkerRule of this.checkers) {
                if (ruleMatches(checkerRule, toolCall, stringifiedArgs, serverName, this.approvalMode, toolAnnotations)) {
    ...
    getExcludedTools(toolMetadata, allToolNames) {
        ...
        for (const rule of this.rules) {
                ...
                for (const [toolName, annotations] of toolMetadata) {
                    ...
                    if (rule.toolName) {
                        if (isWildcardPattern(rule.toolName)) {
                            ...
                            const serverName = typeof rawServerName === 'string' ? rawServerName : undefined;
            ...
            for (const processed of processedTools) {
                if (isWildcardPattern(processed) &&
