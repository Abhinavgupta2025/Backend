...
modes = ["autoEdit"]
`);
            const getPolicyTier2 = (_dir) => 2; // Tier 2
            ...
            const getPolicyTier3 = (_dir) => 3; // Tier 3
...
`);
    ...
    describe('Built-in Plan Mode Policy', () => {
        it('should allow MCP tools with readOnlyHint annotation in Plan Mode (ASK_USER, not DENY)', async () => {
            ...
            const tempPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), 'plan-annotation-test-'));
            try {
                ...
                const annotationRule = result.rules.find((r) => r.toolAnnotations !== undefined);
                expect(annotationRule, 'Should have loaded a rule with toolAnnotations').toBeDefined();
                ...
                const denyRule = result.rules.find((r) => r.decision === PolicyDecision.DENY &&
                ...
                const askResult = await engine.check({ name: 'github__list_issues' }, 'github', { readOnlyHint: true });
                expect(askResult.decision, 'MCP tool with readOnlyHint=true should be ASK_USER, not DENY').toBe(PolicyDecision.ASK_USER);
                // 4. MCP tool WITHOUT annotations should be DENIED
                const denyResult = await engine.check({ name: 'github__create_issue' }, 'github', undefined);
                expect(denyResult.decision, 'MCP tool without annotations should be DENIED in Plan Mode').toBe(PolicyDecision.DENY);
                // 5. MCP tool with readOnlyHint=false should also be DENIED
                const denyResult2 = await engine.check({ name: 'github__delete_issue' }, 'github', { readOnlyHint: false });
                expect(denyResult2.decision, 'MCP tool with readOnlyHint=false should be DENIED in Plan Mode').toBe(PolicyDecision.DENY);
                // 6. Test with qualified tool name format (server__tool) but no separate serverName
                const qualifiedResult = await engine.check({ name: 'github__list_repos' }, undefined, { readOnlyHint: true });
                expect(qualifiedResult.decision, 'Qualified MCP tool name with readOnlyHint=true should be ASK_USER even without separate serverName').toBe(PolicyDecision.ASK_USER);
                // 7. Non-MCP tool (no server context) should be DENIED despite having annotations
                const builtinResult = await engine.check({ name: 'some_random_tool' }, undefined, { readOnlyHint: true });
                expect(builtinResult.decision, 'Non-MCP tool should be DENIED even with readOnlyHint (no server context for *__* match)').toBe(PolicyDecision.DENY);
            }
            finally {
