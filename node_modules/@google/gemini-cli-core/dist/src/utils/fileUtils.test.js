...
import { isWithinRoot, isBinaryFile, detectFileType, processSingleFileContent, detectBOM, readFileWithEncoding, fileExists, readWasmBinaryFromDisk, saveTruncatedToolOutput, formatTruncatedToolOutput, getRealPath, isEmpty, } from './fileUtils.js';
...
<AdapterType Name="ATimeOut" Comment="Adapter for timed events">
  ...
  `;
        ...
        it('should paginate text files correctly (startLine and endLine)', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, tempRootDir, new StandardFileSystemService(), 6, 10); // Read lines 6-10 (1-based)
        ...
        it('should identify truncation when reading the end of a file', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, tempRootDir, new StandardFileSystemService(), 11, 20);
        ...
        it('should handle endLine exceeding file length', async () => {
            ...
            actualNodeFs.writeFileSync(testTextFilePath, lines.join('\n'));
            const result = await processSingleFileContent(testTextFilePath, tempRootDir, new StandardFileSystemService(), 1, 10);
        ...
        it('should truncate when line count exceeds the default limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, tempRootDir, new StandardFileSystemService());
            expect(result.isTruncated).toBe(true);
            expect(result.returnDisplay).toBe('Read lines 1-2000 of 2500 from test.txt');
        ...
        it('should truncate when a line length exceeds the character limit', async () => {
            ...
            const result = await processSingleFileContent(testTextFilePath, tempRootDir, new StandardFileSystemService(), 1, 11);
            expect(result.isTruncated).toBe(true);
    ...
    describe('saveTruncatedToolOutput & formatTruncatedToolOutput', () => {
        it('should save content to a file with safe name', async () => {
            ...
            const expectedOutputFile = path.join(tempRootDir, 'tool-outputs', 'shell_123.txt');
        ...
        it('should sanitize tool name in filename', async () => {
            ...
            const expectedOutputFile = path.join(tempRootDir, 'tool-outputs', '______dangerous_tool_1.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        it('should not duplicate tool name when id already starts with it', async () => {
            ...
            const result = await saveTruncatedToolOutput(content, toolName, id, tempRootDir);
            const expectedOutputFile = path.join(tempRootDir, 'tool-outputs', 'run_shell_command_1707400000000_0.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        it('should sanitize id in filename', async () => {
            ...
            const expectedOutputFile = path.join(tempRootDir, 'tool-outputs', 'shell_______etc_passwd.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        it('should sanitize sessionId in filename/path', async () => {
            ...
            const result = await saveTruncatedToolOutput(content, toolName, id, tempRootDir, sessionId);
            // ../../etc/passwd -> ______etc_passwd
            const expectedOutputFile = path.join(tempRootDir, 'tool-outputs', 'session-______etc_passwd', 'shell_1.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        ...
        it('should format large content with head/tail truncation', () => {
            ...
            expect(formatted).toContain('Showing first 800 and last 3,200 characters');
            expect(formatted).toContain('For full output see: /tmp/out.txt');
